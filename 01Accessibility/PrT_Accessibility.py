...
def potential(origins):
    ...
    arcpy.AddMessage("> origins from "+str(origins)+" to "+str(origins+1000)+" from "+str(Origins))
    arcpy.Delete_management("A_Shape")
    for field in arcpy.Describe(A_Shape).fields:
            if field.type == "OID": OID = str(field.name)
    arcpy.MakeFeatureLayer_management(A_Shape, "A_Shape",OID+" >= "+str(origins)+" and "+OID+" < "+str(origins+1000))
...
if "Potential" in Modus:
    ...
    for origin_l in range(0,Origins,1000): potential(origin_l)
